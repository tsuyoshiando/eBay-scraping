...
credentials = ServiceAccountCredentials.from_json_keyfile_name('scraping-ebay-af00c7cf58d4.json', scope)
gc = gspread.authorize(credentials)

# 「キー」でワークブックを取得
SPREADSHEET_KEY = '1sTG6m41Ferg-jF6yVbyiodvNYrcIOTGAPwc4vCFHeOM'
...
try:
    ...
    api.execute('GetOrders', {
        'CreateTimeFrom' : str(yesterday),
        'CreateTimeTo' : str(today),
    })
    ...
    for order in api.response.reply.OrderArray.Order:
        ...
        for txn in order.TransactionArray.Transaction:
            ...
            price = float(data["order_subtotal"].value) + float(order_shippingCost.value)
